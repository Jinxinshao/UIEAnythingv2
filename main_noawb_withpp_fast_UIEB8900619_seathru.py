...
# 其他项目特定导入
from seathru import run_pipeline # 假设这是主要的CPU处理流程
...
def run(image_path, out_path, depth_out_path, before_post_process_save_path, args_obj, log_file_path):
    ...
    # 创建包含所有必需参数的 seathru_args
    seathru_args = argparse.Namespace(
        p=args_obj.p,
        f=args_obj.f,
        min_depth=args_obj.min_depth,
        max_depth=args_obj.max_depth,
        l=args_obj.l,
        spread_data_fraction=args_obj.spread_data_fraction
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_folder', type=str, default='./output_results_UIEB890_noawb_withpp_fast_UIEB8900619_seathru', help='Folder to save results')
